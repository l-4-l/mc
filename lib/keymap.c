...
static void
mc_keymap_add_keycode_to_group (GTree * keymap_group, const char *name, char *pressed_keynames)
{
...
gboolean
mc_keymap_bind_keycode (const char *group, const char *name, char *pressed_keynames,
                        GError ** error)
{
    GTree *keymap_group;
    ...
    mc_keymap_add_keycode_to_group (keymap_group, name, pressed_keynames);
...
gboolean
mc_keymap_mass_bind_event (mc_keymap_event_init_t * keymap_events, GError ** error)
{
    size_t array_index;

    for (array_index = 0; keymap_events[array_index].group != NULL; array_index++)
    {
        if (!mc_keymap_bind_event (keymap_events[array_index].group,
                                   keymap_events[array_index].name,
